...
@WebAppConfiguration
public class ReportAreaPointConsumeDailyControllerTest {

	private MockMvc mvc;
	
	@Autowired
	private ReportAreaPointConsumeDailyController reportAreaPointConsumeDailyController;
	...
	public void setUp() throws Exception {
		mvc = MockMvcBuilders.standaloneSetup(reportAreaPointConsumeDailyController).build();
	}
	...
	public void insertReportAreaAdPointMonth() {
		...
		RequestBuilder request = post("/reportAreaPointConsumeDaily/insertReportAreaPointConsumeDaily").contentType(MediaType.APPLICATION_JSON).content(requestJson);
		try {
	...
	public void getReportAreaPointConsumeDaily() {
		...
		RequestBuilder request = get("/reportAreaPointConsumeDaily/getReportAreaPointConsumeDaily").param("cityId", "1")
	...
	public void selectReportAreaPointConsumeDailyInMonth() {
		...
		param.setTotalRechargePoint(new BigDecimal(2));
		reportAreaPointConsumeDailyService.insertReportAreaPointConsumeDaily(param);
		RequestBuilder request = get("/reportAreaPointConsumeDaily/selectReportAreaPointConsumeDailyInMonth")
				.param("bdate", DateUtil.getDateFormat(DateUtil.getFirstDayOfMonth(DateUtil.getMonthBefore(new Date()))))
				.param("edate", DateUtil.getDateFormat(DateUtil.getLastDayOfMonth(DateUtil.getMonthBefore(new Date()))));
		try {
			ResultActions perform = mvc.perform(request);
