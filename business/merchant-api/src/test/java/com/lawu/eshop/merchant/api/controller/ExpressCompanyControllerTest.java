...
public class ExpressCompanyControllerTest {
    ...
    @Test
    public void list() {
        RequestBuilder request = get("/expressCompany/list").header("authorization", "8888");
        try {
    ...
    @Test
    public void group() {
        RequestBuilder request = get("/expressCompany/group").header("authorization", "8888");
        try {
            ResultActions perform = mvc.perform(request);
            MvcResult mvcResult = perform.andExpect(status().is(HttpCode.SC_OK)).andDo(MockMvcResultHandlers.print()).andReturn();
            Assert.assertEquals(HttpCode.SC_OK, mvcResult.getResponse().getStatus());
    ...
    public void listByKeyWord() {
        RequestBuilder request = get("/expressCompany/keyWord").header("authorization", "8888").param("keyWord", "test");
        try {
            ResultActions perform = mvc.perform(request);
            MvcResult mvcResult = perform.andExpect(status().is(HttpCode.SC_OK)).andDo(MockMvcResultHandlers.print()).andReturn();
            Assert.assertEquals(HttpCode.SC_OK, mvcResult.getResponse().getStatus());
