...
public class RechargeControllerTest {
    ...
    public void save() {
        ...
        map.add("rechargeMoney", "10");
        map.add("payTypeEnum", "BALANCE");
        map.add("transactionPayTypeEnum", "BALANCE");
        map.add("payPwd", "123456");
        RequestBuilder request = post("/recharge/save").header("authorization", "8888").params(map);
        try {
    ...
    public void getRechargeById() {
        RequestBuilder request = get("/recharge/getRechargeById/10").header("authorization", "8888");
        try {
            ResultActions perform = mvc.perform(request);
