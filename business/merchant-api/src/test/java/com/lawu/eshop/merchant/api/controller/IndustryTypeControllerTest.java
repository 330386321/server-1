...
public class IndustryTypeControllerTest {
    ...
    @Test
    public void getIndustryListWithGroup() {
        RequestBuilder request = get("/industryType/getIndustryListWithGroup");
        try {
    ...
    public void listIndustryTypeByType() {
        RequestBuilder request = get("/industryType/listIndustryTypeByType").param("industryTypeEnum", "COMMON_INDUSTRY");
        try {
            ResultActions perform = mvc.perform(request);
            MvcResult mvcResult = perform.andExpect(status().is(HttpCode.SC_OK)).andDo(MockMvcResultHandlers.print()).andReturn();
            Assert.assertEquals(HttpCode.SC_OK, mvcResult.getResponse().getStatus());
