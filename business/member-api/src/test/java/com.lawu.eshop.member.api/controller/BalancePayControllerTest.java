...
public class BalancePayControllerTest extends BaseController {
    ...
    public void setUp() throws Exception {
        mvc = MockMvcBuilders.standaloneSetup(balancePayController).addInterceptors(authorizationInterceptor).build();
    }
    ...
    public void orderPay() {
        ...
        params.add("bizIds", "1");
        RequestBuilder request = post("/balancePay/orderPay").header("authorization","").params(params);
        try {
    ...
    public void billPay() {
        MultiValueMap<String, String> params = new LinkedMultiValueMap<String, String>();
        params.add("bizIds", "1");
        RequestBuilder request = post("/balancePay/billPay").header("authorization","").params(params);
        try {
            ResultActions perform = mvc.perform(request);
            MvcResult mvcResult = perform.andExpect(status().is(HttpCode.SC_CREATED)).andDo(MockMvcResultHandlers.print()).andReturn();
        } catch (Exception e) {
    ...
    public void balancePayPoint() {
        MultiValueMap<String, String> params = new LinkedMultiValueMap<String, String>();
        params.add("bizIds", "1");
        RequestBuilder request = post("/balancePay/balancePayPoint").header("authorization","").params(params);
        try {
            ResultActions perform = mvc.perform(request);
            MvcResult mvcResult = perform.andExpect(status().is(HttpCode.SC_CREATED)).andDo(MockMvcResultHandlers.print()).andReturn();
        } catch (Exception e) {
