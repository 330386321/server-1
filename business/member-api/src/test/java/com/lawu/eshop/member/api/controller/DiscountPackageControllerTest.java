...
public class DiscountPackageControllerTest {
    ...
    public void setUp() throws Exception {
        mvc = MockMvcBuilders.standaloneSetup(discountPackageController).addInterceptors(authorizationInterceptor).build();
    }

    @Test
    public void list() {
        RequestBuilder request = get("/discountPackage/list/1").param("merchantId","1");
        try {
    ...
    @Test
    public void getById() {
        RequestBuilder request = get("/discountPackage/1").param("id","1");
        try {
            ResultActions perform = mvc.perform(request);
            MvcResult mvcResult = perform.andExpect(status().is(HttpCode.SC_OK)).andDo(MockMvcResultHandlers.print()).andReturn();
        } catch (Exception e) {
