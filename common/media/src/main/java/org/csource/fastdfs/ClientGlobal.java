...
public class ClientGlobal {
	 ...
	 *            config filename
	 */
	public static void init(ClientParams param) throws FileNotFoundException, IOException, MyException {
		String[] szTrackerServers;
		String[] parts;

		g_connect_timeout = param.getConnectTimeout();
		if (g_connect_timeout < 0) {
			g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
		}
		g_connect_timeout *= 1000; // millisecond

		g_network_timeout = param.getNetworkTimeout();
		if (g_network_timeout < 0) {
			g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
		}
		g_network_timeout *= 1000; // millisecond

		g_charset = param.getCharset();
		if (g_charset == null || g_charset.length() == 0) {
			g_charset = "ISO8859-1";
		}
		String tserver = param.getTrackerServer();
		szTrackerServers = tserver.split(",");
		if (szTrackerServers == null) {
			throw new MyException("item \"tracker_server\" 配置项未找到 ");
		}
		...
		if (g_anti_steal_token) {
			g_secret_key = param.getSecretKey();// iniReader.getStrValue("http.secret_key");
		}
	}
